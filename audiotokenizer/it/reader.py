...
*not* write is the strongest cheap check that the byte layout is correct. This module is a thin wrapper:
it dumps every sample to WAV in a temp dir and reads them back as float PCM. It depends on the dev-only
``xmodits-py`` and stays out of the package's public surface, so a runtime install without the validation
tooling still imports cleanly.
"""

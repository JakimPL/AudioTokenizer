"""Impulse Tracker's view of the shared tracker clock: ``speed``/``tempo`` -> exact integer row length.

The integrality logic lives in :mod:`audiotokenizer.tracker.timing`; this module only binds Impulse
Tracker's ranges (``MIN_SPEED``/``MIN_TEMPO``/``MAX_TEMPO`` from :mod:`audiotokenizer.it.spec`) so
callers work in IT terms. See the shared module for the derivation of ``row_frames``.
"""

from __future__ import annotations

from audiotokenizer.it.spec import MAX_TEMPO, MIN_SPEED, MIN_TEMPO
from audiotokenizer.tracker.timing import Timing
from audiotokenizer.tracker.timing import exact_timings as _exact_timings
from audiotokenizer.tracker.timing import nearest_timing as _nearest_timing
from audiotokenizer.tracker.timing import row_frames as _row_frames

__all__ = ["Timing", "row_frames", "exact_timings", "nearest_timing"]


def row_frames(speed: int, tempo: int, *, frame_rate: int, max_tempo: int = MAX_TEMPO) -> int:
    """Return the exact frames in one IT row, or raise when ``(speed, tempo)`` gives a fractional row."""
    return _row_frames(
        speed, tempo, frame_rate=frame_rate, min_speed=MIN_SPEED, min_tempo=MIN_TEMPO, max_tempo=max_tempo
    )


def exact_timings(*, frame_rate: int, speed: int = MIN_SPEED) -> list[Timing]:
    """Every IT tempo whose row length is a whole number of frames at ``speed``, sorted by row length."""
    return _exact_timings(frame_rate=frame_rate, min_tempo=MIN_TEMPO, max_tempo=MAX_TEMPO, speed=speed)


def nearest_timing(target_frames: int, *, frame_rate: int, speed: int = MIN_SPEED) -> Timing:
    """The exact IT-row timing whose length is closest to ``target_frames`` (ties -> the shorter row)."""
    return _nearest_timing(target_frames, frame_rate=frame_rate, min_tempo=MIN_TEMPO, max_tempo=MAX_TEMPO, speed=speed)

"""Render tracker modules to audio with ``openmpt123`` — the ground-truth playback engine, shared by formats.
...
``.xm``, …), so the driver here is format-agnostic: it only needs the file's bytes and its extension. Each
format's ``render`` module binds this to its own writer.

...
looks up (``keymap[note - 1]``). Envelopes are left disabled, so a one-shot sample plays at its sample
volume and falls silent when it ends — exactly at the row boundary, since it is one row long.
"""
...
def note_keymap(n_samples: int) -> bytes:
    """A 96-byte keymap routing key ``NOTE_BASE + k`` to the instrument's ``(k + 1)``-th sample.

    Indexed by the 0-based note the player derives from the 1-based pattern note (``keymap[note - 1]``);
    unmapped keys stay ``0`` (no sample).
    """
    ...
    for slot in range(n_samples):
        keymap[play_note_for(slot) - 1] = slot + 1  # 1-based sample within the instrument
    return bytes(keymap)

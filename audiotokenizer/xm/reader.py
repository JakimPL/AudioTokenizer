"""Round-trip reader for ``.XM`` files, used to validate the hand-rolled writer.

``xmodits`` is an independent, Rust-backed sample ripper; decoding a file we wrote with a parser we did
*not* write is the strongest cheap check that the byte layout is correct. This mirrors
:mod:`audiotokenizer.it.reader`: it dumps every sample to WAV in a temp dir and reads them back as float
PCM. It depends on the dev-only ``xmodits-py`` and stays out of the package's public surface, so a runtime
...
class RippedSample:
    """One sample recovered from an ``.XM`` file: its index, name, tagged rate and float PCM."""
...
def read_xm_samples(path: Path | str) -> list[RippedSample]:

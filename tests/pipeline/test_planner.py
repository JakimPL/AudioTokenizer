...
from audiotokenizer.pipeline.planner import plan_compilation
...
def test_plan_fits_the_budget_and_is_a_real_module(signal: NDArray[np.float64]) -> None:
    budget = 400 * 1024
    plan = plan_compilation(signal, budget_bytes=budget, profiles=("strict",), n_atoms_grid=_GRID)
    assert plan.module.within_budget
    assert plan.module.cost.total <= budget
    # The chosen config is compiled into a genuine module: its size model equals the bytes it writes.
    assert plan.module.cost.total == len(plan.module.to_bytes())


def test_plan_is_no_worse_than_a_hand_config_in_its_search_space(signal: NDArray[np.float64]) -> None:
    hand = compile_signal(signal, TokenizerConfig(profile="strict", tempo=125, n_atoms=8, budget_bytes=_BIG_BUDGET))
    plan = plan_compilation(signal, budget_bytes=_BIG_BUDGET, profiles=("strict",), n_atoms_grid=_GRID)
    # The hand config (strict, tempo 125, 8 atoms) is one point the sweep visits, so the winner cannot lose.
...
def test_frontier_is_a_monotone_pareto_ending_in_the_choice(signal: NDArray[np.float64]) -> None:
    plan = plan_compilation(signal, budget_bytes=_BIG_BUDGET, profiles=("strict", "hacked"), n_atoms_grid=_GRID)
...
def test_nothing_fits_returns_the_closest_module_instead_of_discarding_the_run(signal: NDArray[np.float64]) -> None:
    ...
    # render.
    plan = plan_compilation(signal, budget_bytes=1024, profiles=("strict",), n_atoms_grid=_GRID)
    assert not plan.within_budget
    assert plan.module.cost.total > 1024
    assert plan.module.cost.total == len(plan.module.to_bytes())  # a genuine module, not a stub
...
def test_plan_raises_only_when_there_is_nothing_to_compile(signal: NDArray[np.float64]) -> None:
    ...
    with pytest.raises(ValueError):
        plan_compilation(signal, budget_bytes=_BIG_BUDGET, profiles=("strict",), n_atoms_grid=())


def test_planner_only_ever_returns_a_writable_module(signal: NDArray[np.float64]) -> None:
    # Fitting the byte budget is not enough — a pattern over IT's u16 length field cannot be serialized.
    # The planner must reject those, so the module it hands back always writes without raising.
    plan = plan_compilation(signal, budget_bytes=_BIG_BUDGET, profiles=("strict", "hacked"), n_atoms_grid=_GRID)
    assert plan.module.cost.writable
    assert plan.module.cost.total == len(plan.module.to_bytes())

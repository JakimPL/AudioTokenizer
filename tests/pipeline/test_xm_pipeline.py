...
def test_channel_cap_is_format_and_compliance_aware() -> None:
    ...
    assert TokenizerConfig(compliance=EXTENDED, format=Format.XM, tempo=125, n_atoms=10).max_channels == 127
...
def test_tempo_ceiling_reaches_past_a_byte_only_where_the_field_is_a_word() -> None:
    # This format's header tempo is a word, so extending it reaches rows the canonical byte cannot ask for.
    assert TokenizerConfig(compliance=EXTENDED, format=Format.XM, tempo=125, n_atoms=10).max_tempo == (
        EXTENDED_MAX_TEMPO
    )
    assert TokenizerConfig(compliance=CANONICAL, format=Format.XM, tempo=125, n_atoms=10).max_tempo == (
        CANONICAL_MAX_TEMPO
    )
    # Impulse Tracker's is a byte at every level, which is the bug the limit table exists to prevent.
...
def test_atom_pool_cap_is_wider_for_xm() -> None:
    assert max_atoms_for(Format.IT, CANONICAL) == 49
    assert max_atoms_for(Format.IT, EXTENDED) == 127
    assert max_atoms_for(Format.XM, EXTENDED) == 2040
    # A pool IT cannot route (200 atoms -> 400 stored samples > 99) is fine for XM (400 <= 2048).
...
def test_xm_module_round_trips_through_xmodits(signal: NDArray[np.float64], tmp_path: Path) -> None:
    ...
    assert len(ripped) == len(compiled.module.song.voices.samples) == POLARITIES * compiled.n_atoms_used
...
def test_extended_xm_writes_more_channels_than_the_tracker_honours(signal: NDArray[np.float64]) -> None:
    # The extended bound is what the players descended from FastTracker 2 read, 127 channels. The same
    # pool that the canonical level clamps to 32 channels spreads over more of them here.
...
def test_pattern_count_never_needs_more_orders_than_the_table_holds() -> None:
    ...
    # 256 rows split into 256 one-row patterns, and the largest canonical song fills the whole order table.
    assert XM_BINDING.pattern_count(CANONICAL_MAX_PATTERNS) == CANONICAL_MAX_PATTERNS
    assert XM_BINDING.pattern_count(CANONICAL_MAX_PATTERNS * MAX_ROWS) == CANONICAL_MAX_PATTERNS


@pytest.mark.parametrize("rows", [1, 7, 255, 256, 257, 530, 4097, CANONICAL_MAX_PATTERNS * MAX_ROWS])
def test_pattern_count_covers_every_row_within_both_ceilings(rows: int) -> None:
    ...
    height = -(-rows // count)
    assert 1 <= count <= CANONICAL_MAX_PATTERNS
...
def test_a_song_past_the_wall_leaves_a_pattern_over_the_row_ceiling() -> None:
    ...
    rows = CANONICAL_MAX_PATTERNS * MAX_ROWS + 1
    count = XM_BINDING.pattern_count(rows)
    assert count == CANONICAL_MAX_PATTERNS

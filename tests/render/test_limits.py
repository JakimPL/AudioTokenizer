...
long as the row clock says it should. A 192-channel module that quietly dropped its extra channels, or a
...
from trackmod.it.spec.ranges import EXTENDED_MAX_CHANNELS as IT_EXTENDED_MAX_CHANNELS
...
from trackmod.xm.spec.ranges import EXTENDED_MAX_CHANNELS as XM_EXTENDED_MAX_CHANNELS
...
def test_it_plays_127_channels() -> None:
    ...
    assert_plays_the_clock(module, rows=PROBE_ROWS, row_frames=it_row_frames(speed, tempo, frame_rate=RATE))


def test_xm_plays_192_channels() -> None:
    # 32 is what FastTracker 2 honoured; 192 is where libopenmpt stops loading, which is why the extended
    # bound is 192 rather than the 255 the header's channel word could hold.
    speed, tempo = 6, 125
    ...
    assert module.violations() == ()
    assert_plays_the_clock(module, rows=PROBE_ROWS, row_frames=xm_row_frames(speed, tempo, frame_rate=RATE))
...
def test_xm_plays_a_tempo_past_the_byte_a_canonical_module_stores() -> None:
    ...
    speed, tempo = 1, 441
    row_frames = xm_row_frames(speed, tempo, frame_rate=RATE)
